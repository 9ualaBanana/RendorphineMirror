...
{
    public static NamedList<T> CreateRaw<T>(LocalizedString title, IReadOnlyCollection<T> items, Func<T, IControl> templatefunc) => Create(title, new Bindable<IReadOnlyCollection<T>>(items), templatefunc);
    public static NamedList<T> Create<T>(LocalizedString title, IReadOnlyBindable<IReadOnlyCollection<T>> items, Func<T, IControl> templatefunc) => new(title, items, templatefunc);
}
