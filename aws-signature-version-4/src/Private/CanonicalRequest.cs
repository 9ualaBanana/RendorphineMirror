...
namespace AwsSignatureVersion4.Private
{
    ...
    public static class CanonicalRequest
    {
            ...
            string contentHash)
        {
            ...
            var sortedParameters = SortQueryParameters(request.RequestUri.Query);
            var keyValueParameters = new List<string>();
            foreach (var parameter in sortedParameters)
            {
                if (parameter.Value.Any()) keyValueParameters.AddRange(sortedParameters[parameter.Key].Select(
                        parameterValue => $"{AWSSDKUtils.UrlEncode(parameter.Key, false)}={AWSSDKUtils.UrlEncode(parameterValue, false)}")
                    );
                else keyValueParameters.Add($"{parameter.Key}=");
            }

            builder.Append($"{string.Join("&", keyValueParameters)}\n");
        ...
        public static SortedList<string, List<string>> SortQueryParameters(string query)
        {
            ...
            {
                var key = (parameterName ?? queryParameters[parameterName])!;
                // Create query parameter if it doesn't already exist
                if (!sortedQueryParameters.TryGetValue(key, out var parameterValues))
                {
                    parameterValues = new List<string>();
                    sortedQueryParameters.Add(key, parameterValues);
                }

                var queryParameterValues = queryParameters.GetValues(key);
