    ...
    where ECallbackData : struct, Enum
{
    ...
    protected abstract Task HandleAsync(TCallbackQuery callbackQuery, CachedMediaFile cachedMediaFile);


    protected async Task UploadToMPlusAsync(CachedMediaFile cachedMediaFile)
    {
        ...
        try { await PacketsTransporter.UploadAsync(new MPlusUploadSessionData(cachedMediaFile.Path, MPlusIdentity.SessionIdOf(User)), _httpClient); }
