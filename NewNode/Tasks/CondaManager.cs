...
public static class CondaManager
{
    ...
    public static string WrapWithInitEnv(string condapath, string name, string script) => $"""
        {GetInitEnvScript(condapath, name, _ => throw new Exception("Conda environment was not initialized"))}
        {script}
        """;

    static string GetInitEnvScript(string condapath, string name, Func<string, string> createEnvFunc)
    {
        ...
        return $"""
            # initialize conda
            (& "{condapath}" "shell" "hook" "--shell=powershell") | Out-String | ?{"{$_}"} | Invoke-Expression
            {hookfix}
    ...
    public static void Initialize(string condapath, string name, string pyversion, IEnumerable<string> requirements, IEnumerable<string> channels, IEnumerable<string>? piprequirements)
    {
        var script = $"""
            {GetInitEnvScript(condapath, name, envdir => $"micromamba create -y -p '{envdir}' python={pyversion} {string.Join(' ', channels.Select(c => $"-c '{c}'"))}")}
