...
public class MPlusTaskInputInfo : ITaskInputInfo
{
    ...
    static ValueTask<OperationResult<Dictionary<string, MPlusNewItem>>> GetMpItems(Api api, string sessionid, string userid, IEnumerable<string> iids) =>
    ...
    public ValueTask<OperationResult<TaskObject>> GetFileInfo(Api api, string sessionid, string myuserid) => GetFileInfo(api, sessionid, TUid ?? myuserid, Iid);
    public static ValueTask<OperationResult<TaskObject>> GetFileInfo(Api api, string sessionid, string userid, string iid) =>
    ...
    public static ValueTask<OperationResult<Dictionary<string, TaskObject>>> GetFilesInfoDict(Api api, string sessionid, string userid, IEnumerable<string> iids) =>
        GetMpItems(api, sessionid, userid,iids).Next(data => data.Values.Select(item => KeyValuePair.Create(item.Iid, new TaskObject(item.Files.File.FileName, item.Files.File.Size)).AsOpResult()).MergeDictResults());
}
